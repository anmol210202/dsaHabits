...
public:
    vector<vector<int>> insert(vector<vector<int>>& intervals, vector<int>& newInterval) {
        ...
        for(int i=1; i<100000; i++) v[i].first+=v[i-1].first;
        if(newInterval[0]==newInterval[1] and v[newInterval[0]].first==0) v[newInterval[0]].second=true;
        vector<int> tmp;
        if(v[0].first!=0 ) tmp.push_back(0);
        if(v[0].second and v[0].first==0) ans.push_back({0,0});
        for(int i=1; i<100000-1; i++) {
            if(v[i].second and v[i].first ==0 and v[i-1].first==0) ans.push_back({i ,i});
